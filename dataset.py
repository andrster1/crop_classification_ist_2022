...
class PixelSetDataDifferences(data.Dataset):
    def __init__(self, folder, labels, npixel, sub_classes=None, norm=None,
        ...
        self.date_positions = np.array(date_positions(self.dates))
    ...
    def __getitem__(self, item):
        """
        Returns:
              original data in ((Pixel-Set, Pixel-Mask), Extra-features) format
              differences data in ((Pixel-Set, Pixel-Mask), Extra-features) format
              original data dates
              labels
        """
        ...
        intermediate_x = Tensor(x.copy()).permute(1, 0, 2)
        x_dif = [intermediate_x[:, i + 1] - intermediate_x[:, i] for i in range(0, intermediate_x.size(1) - 1)]
        x_dif = torch.stack(x_dif, 0)
        x_dif_mask = np.stack([inter_mask for _ in range(x_dif.size(0))], axis=0)  # Add temporal dimension to mask

        if self.extra_feature is not None:
            ef = (self.extra[str(self.pid[item])] - self.extra_m) / self.extra_s
            ef_inter = torch.from_numpy(ef).float()
            ef = torch.stack([ef_inter for _ in range(data[0].shape[0])], dim=0)
            ef_dif = torch.stack([ef_inter for _ in range(x_dif.size(0))], dim=0)
            data = (data, ef)
